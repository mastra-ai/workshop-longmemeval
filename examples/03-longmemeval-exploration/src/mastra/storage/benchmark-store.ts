...
export class BenchmarkStore extends MastraStorage {
  ...
  private mode: DBMode;
  private resourceLogTimestamps: Map<string, number> = new Map();
  ...
  async getThreadById({ threadId }: { threadId: string }): Promise<StorageThreadType | null> {
    ...
    if (thread) {
      // Try to get the resource to find metadata
      const resource = this.data.mastra_resources.get(thread.resourceId);
      
      if (resource?.metadata?.question && resource?.metadata?.answer) {
        const now = Date.now();
        const lastLogTime = this.resourceLogTimestamps.get(threadId) || 0;
        
        // Only log if it's been more than 5 seconds since last log
        if (now - lastLogTime > 5000) {
          console.log('\n📖 Thread loaded:', threadId);
          console.log('🏷️  Type:', resource.metadata.questionType || 'unknown');
          console.log('❓ Question:', resource.metadata.question);
          console.log('✅ Answer:', resource.metadata.answer);
          console.log('');
          
          this.resourceLogTimestamps.set(threadId, now);
        }
      }
    }
    
    return thread ? {...thread, title: thread.id}: null;
  }
  ...
  async getThreadsByResourceId({ resourceId }: { resourceId: string }): Promise<StorageThreadType[]> {
    ...
    for (const thread of this.data.mastra_threads.values()) {
      if (thread.resourceId === resourceId) {
        threads.push({...thread, title: thread.id});
      }
    }
    return threads;
  }
  ...
  async updateResource({
    ...
    // Always update in-memory data, even in read mode
  ...
  // Missing scorer-related methods
  async dropTable(_: { tableName: TABLE_NAMES }): Promise<void> {
    // Not implemented for in-memory storage
  }

  async getScoreById({ id }: { id: string }): Promise<any | null> {
    throw new Error('getScoreById not implemented');
  }

  async saveScore({ score }: { score: any }): Promise<any> {
    this.data.mastra_scorers.set(score.id, score);
    return score;
  }

  async getScoresByScorerId({ scorerId, pagination, entityId, entityType }: { 
    scorerId: string; 
    pagination: any; 
    entityId?: string; 
    entityType?: string; 
  }): Promise<{ pagination: any; scores: any[] }> {
    throw new Error('getScoresByScorerId not implemented');
  }

  async updateScore({ scoreId, score }: { scoreId: string; score: Partial<any> }): Promise<any> {
    const existing = this.data.mastra_scorers.get(scoreId);
    if (!existing) return existing;

    const updated = { ...existing, ...score, id: scoreId };
    this.data.mastra_scorers.set(scoreId, updated);
    return updated;
  }

  async getScoresByResourceId({ resourceId }: { resourceId: string }): Promise<any[]> {
    const scores: any[] = [];
    for (const score of this.data.mastra_scorers.values()) {
      if (score.resourceId === resourceId) {
        scores.push(score);
      }
    }
    return scores;
  ...
  async deleteScore({ scoreId }: { scoreId: string }): Promise<void> {
    this.data.mastra_scorers.delete(scoreId);
  }

  async getScoresByRunId(_: { runId: string; pagination: any }): Promise<{ pagination: any; scores: any[] }> {
    throw new Error('getScoresByRunId not implemented');
  }

  async getScoresByEntityId(_: { entityId: string; entityType: string; pagination: any }): Promise<{ pagination: any; scores: any[] }> {
    throw new Error('getScoresByEntityId not implemented');
  }

  async getEvals(_: any): Promise<any> {
    throw new Error('getEvals not implemented');
  }
}
